...
export class ChatBoxVisitor extends cslVisitor<ChatBoxStmt> {
  ...
  /**
   * Visits a ChatBox statement.
   * @param ctx The Chatbox_stmtContext object.
   * @returns The ChatBoxStmt object.
   */
  override visitChatbox_stmt = (ctx: Chatbox_stmtContext): ChatBoxStmt => {
    let default_branch = this.command_visitor_.visitDefault_branch(
      ctx.default_branch()
    ) as CaseStmt;
    ...
    let case_list = this.command_visitor_.visitCase_list(
      case_list_stmt
    ) as CaseStmt[];
